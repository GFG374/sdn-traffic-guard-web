...
class SDNSecurityController(app_manager.RyuApp):
    ...
    def packet_in(self, ev):
        ...
        if arp_pkt:
            ...
            self.arp_table[dpid][eth.src] = src_ip
        else:
            ip_pkt = pkt.get_protocol(ipv4.ipv4)
            if ip_pkt:
                src_ip = ip_pkt.src
...
# -----------------------------------------------------------
    ...
    def _device_anomaly_detection_loop(self):
        ...
        while True:
            try:
                ...
                # 每30秒（3个10秒周期）检查一次已知主机的IP异常
                if check_counter >= 3:
                    self._check_known_hosts_ip_anomaly()
                    check_counter = 0
    ...
    def _check_known_hosts_ip_anomaly(self):
        """
        ✅ 检查已知主机的IP是否异常
        - 遍历所有见过的MAC地址
        - 检查每个MAC对应的IP是否在合法网段
        ...
        """
        try:
            self.logger.info(f"🔍 [_check_known_hosts_ip_anomaly] 开始检查已知主机的IP异常")
            
            # 遍历所有交换机的MAC-端口映射
            for dpid, mac_dict in self.mac_to_port.items():
                self.logger.debug(f"🔍 [_check_known_hosts_ip_anomaly] 检查交换机{dpid}，已知MAC数: {len(mac_dict)}")
                
                # 对于每个MAC，我们需要查询其对应的IP
                # 由于我们在packet_in中已经记录了MAC-IP对应关系，
                # 这里我们可以从flow_cache或arp_table中查询
                for mac, port in mac_dict.items():
                    # 从ARP表中查找该MAC对应的IP
                    if dpid in self.arp_table and mac in self.arp_table[dpid]:
                        ip = self.arp_table[dpid][mac]
                        # 检查这个IP是否在合法网段
                        self._check_ip_subnet(ip)
            
            self.logger.info(f"✅ [_check_known_hosts_ip_anomaly] 已知主机IP异常检查完成")
            
        except Exception as e:
            self.logger.error(f"❌ [_check_known_hosts_ip_anomaly] 检查失败: {e}")
            import traceback
            traceback.print_exc()
...
# 需要在 sdn_smart.py 中替换原有的 get_anomalies 方法
    ...
    def update_device_anomaly_status(self, req, anomaly_id, **_):
        ...
        将状态从 pending 改为 handled，并填充 handled_by 和 handled_at
        """
        try:
            ...
            # 解析请求体
            body = json.loads(req.body.decode('utf-8'))
            status = body.get('status', 'handled')  # 默认改为 'handled'
            ...
            try:
                ...
                with conn.cursor() as cur:
                    # ✅ 更新异常状态为已处理（handled）
                    # 填充 resolved_at、handled_by、handled_at、handle_action 字段
                    cur.execute("""
                        ...
                        SET status = %s,
                            ...
                            handle_action = %s
                        WHERE id = %s
                    """, ('handled', 'admin', 'marked_resolved', anomaly_id))
                    
                    affected_rows = cur.rowcount
                    
                    if affected_rows > 0:
                        self.ctrl.logger.info(f"✅ 异常#{anomaly_id}已标记为已处理（handled）")
                        self.ctrl.logger.info(f"   - status: pending → handled")
                        self.ctrl.logger.info(f"   - handled_by: admin")
                        self.ctrl.logger.info(f"   - handled_at: NOW()")
                        self.ctrl.logger.info(f"   - handle_action: marked_resolved")
                        return self._json_resp({
                            'success': True,
                            'message': f'异常已标记为已处理',
                            'affected_rows': affected_rows,
                        ...
                        self.ctrl.logger.warning(f"⚠️ 异常#{anomaly_id}不存在或已处理")
                        return self._json_resp({
                            'success': True,
                            'message': f'异常已处理或不存在',
                            'affected_rows': 0
